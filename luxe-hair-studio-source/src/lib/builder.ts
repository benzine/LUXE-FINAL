...
export const builderStore = {
  ...
  /* Isolation mode */
  enterIsolatedMode: (uid: string) => {
    const sectionId = builderStore.slots?.find((s: any) => s.uid === uid)?.id || "";
    state = { 
      ...state, 
      isolatedSectionUid: uid,
      viewMode: "isolated",
      selection: { type: "section", sectionUid: uid, sectionId }
    };
    emit();
  },
  exitIsolatedMode: () => {
...
// Temporary reference to slots - will be passed from config
builderStore.slots = undefined as any;

export function useBuilder(): BuilderState & typeof builderStore {
  return useSyncExternalStore(
    builderStore.subscribe,
    () => ({ ...state, ...builderStore }),
    () => ({ ...DEFAULT_STATE, ...builderStore })
  );
}
