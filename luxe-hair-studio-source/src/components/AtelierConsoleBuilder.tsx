...
import { builderStore, useBuilder, setupBuilderKeyboardShortcuts } from "../lib/builder";
...
export default function AtelierConsoleBuilder({ onClose }: { onClose: () => void }) {
  ...
  const handleSelectSection = useCallback((uid: string, id: string) => {
    builder.selectSection(uid, id);
    builder.enterIsolatedMode(uid);
  }, []);
      ...
      <main className="relative z-20 flex-1 bg-[#241c1d]">
        ...
        {builder.viewMode === "isolated" && isolatedSection && (
          <SectionToolbar
            ...
            onBack={() => builder.exitIsolatedMode()}
            onRename={() => toast("Rename functionality")}
            onDuplicate={() => { configStore.duplicateSlot(isolatedSection.uid); toast("Section duplicated"); }}
            onDelete={() => { configStore.removeSlot(isolatedSection.uid); builder.exitIsolatedMode(); toast("Section deleted"); }}
                  ...
                  <div className="mt-6 grid grid-cols-3 gap-4">
                    {[1, 2, 3].map((i) => (
                      <div 
                        ...
                        onClick={() => builder.selectElement(isolatedSection!.uid, isolatedSection!.id, `element-${i}`)}
                      >
