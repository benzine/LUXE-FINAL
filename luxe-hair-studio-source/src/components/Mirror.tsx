              ...
              <div className="mt-3 flex flex-wrap gap-3">
                {mcfg.models.map((m) => (
                  <button key={m.id} onClick={() => { setCustom(null); setMuseId(m.id); }} data-cursor="hand"
                    ...
                    <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0b0808]/85 to-transparent px-1 pb-1 pt-4 text-center font-mono text-[8px] uppercase tracking-[0.08em] text-[#f5e6d3]">{m.label ? m.label.split("·")[0] : ""}</span>
                  </button>
                ))}
